...
function render(breakdown, superAmt, grossSalary){
  ...
  document.getElementById('taxBar').innerHTML = segs.map(s =>
    `<div class="tax-bar-seg" style="width:${(s.amt/salaryForTax*100).toFixed(2)}%; background:${s.color};" title="${s.label}: ${formatAUD(s.amt)}"></div>`
  ).join('');

  document.getElementById('taxLegend').innerHTML = segs.map(s =>
    `<div class="tax-legend-item"><span class="tax-legend-dot" style="background:${s.color};"></span>${s.label}: <strong>${formatAUD(s.amt)}</strong> (${(s.amt/salaryForTax*100).toFixed(1)}%)</div>`
  ).join('');

  document.getElementById('txNet').textContent = formatAUD(net);
  document.getElementById('txNetWeekly').textContent = formatAUD(net / 52);
  document.getElementById('txEffRate').textContent = ((taxAfterLito + medicareAmt + mlsAmt) / salaryForTax * 100).toFixed(1) + '%';
...
function calculateTax(){
  const income = parseFloat(document.getElementById('txIncome').value) || 0;
  ...
  const children = parseInt(document.getElementById('txChildren').value) || 0;
  const sacrifice = Math.max(0, parseFloat(document.getElementById('txSacrifice').value) || 0);
